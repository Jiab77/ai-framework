...
**When in doubt about a request:**
- Read `HUMAN.md` first — the answer is likely already there
- If `HUMAN.md` does not exist or cannot be read, stop and ask the human immediately
- Never guess, never assume, never proceed blind
...
**6. Debugging Protocol (non-negotiable)**
- When a bug is reported, **add extensive debugging logs first** before making any code change
- Logs must cover every layer involved: client, server, API, storage — not just the suspected layer
- Read and analyze the logs **before** writing a single fix
- One wrong fix costs more credits than thorough logging ever will
- If the first fix does not resolve the issue, add more logs — never guess twice in a row
- Remove all debug logs once the root cause is confirmed and the fix is verified
